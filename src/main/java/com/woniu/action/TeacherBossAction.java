...
public class TeacherBossAction extends ActionSupport{
	...
	public String allTeacherAndOverman() {
		...
		Iterator<Teacher> ite = teachers.iterator();
		while(ite.hasNext()) {
			Teacher teacher = ite.next();
			if(teacher.getClazzs().size()!=0) {
				ite.remove();
			}
		}
		overmans = overmanService.findAll();
		Iterator<Overman> ites = overmans.iterator();
		while(ites.hasNext()) {
			Overman overman = ites.next();
			if(overman.getClazzs().size()!=0) {
				ites.remove();
			}
		}
	...
	public String endOneClazzById() {
		clazzService.endOneById(clazzId);
		return "endOneClazzById";
	}
